...
function Options({ authToken, adminId, quizId, questionText }) {
  ...
  const fetchOptions = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5116/api/option/${questionId}/getAllOptions`, {
        method: "GET",
  ...
  const handleCreateOption = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5116/api/option/${questionId}/addOption`, {
        method: "POST",
        headers: { Authorization: `Bearer ${authToken}` },
        body: form,
      });
  ...
  const handleDeleteOption = async (deleteOptionId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5116/api/option/${deleteOptionId}/deleteOption`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${authToken}` },
      });
