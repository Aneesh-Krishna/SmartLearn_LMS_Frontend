...
function Quiz({ authToken, adminId, courseId, courseName, setQuizId, quizTitle, setQuizTitle }) {
  ...
  const fetchQuizzes = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5116/api/quiz/${courseId}/getallquiz`, {
        method: "GET",
  ...
  const handleCreateQuiz = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5116/api/quiz/${courseId}/createQuiz`, {
        method: "POST",
        headers: {
  ...
  const handleDeleteQuiz = async (quizId) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5116/api/quiz/${quizId}/DeleteQuiz`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${authToken}` },
      });
  ...
  const handleGenerateReport = async (generateReportQuizId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5116/api/report/${generateReportQuizId}/GenerateReport`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
