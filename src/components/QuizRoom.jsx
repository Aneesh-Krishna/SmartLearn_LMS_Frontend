...
function QuizRoom({ authToken, quizTitle }) {
  ...
  const fetchQuestionsWithOptions = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5116/api/question/${quizId}/getallquestionswithoptions`, {
        method: "GET",
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5116/api/quizresponse/${quizId}/SubmitQuiz`, {
        method: "POST",
        headers: {
